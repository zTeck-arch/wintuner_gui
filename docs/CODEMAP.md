...
| `45-Assignments` | 1129 | Zuweisungen schreiben: **`Move-AppAssignments`** (seit 0.20.1 der EINZIGE Weg, auf dem die Ablöse den Geltungsbereich umzieht — `-SourceIsAuthoritative` für den Update-Weg, `-Outcome` sagt dem Aufrufer, ob wirklich geschrieben wurde), `Clear-AppAssignments`, `New-AppAssignmentConfiguration`, `Set-AppAssignmentSettings`, `Save-AppScopeSnapshot`, `Enable-AppAutoUpdateChecked` |
| `50-UpdateEngine` | 1291 | **`Update-SingleApp`** (der Kern: paketieren → hochladen → Zuweisungen → Aufräumen), `Invoke-ExistingTargetConsolidation` (der Weg „Zielversion liegt schon im Tenant"), die Urteile als reine Rechnung (`Get-ConsolidationDeleteVerdict`, `Get-UnusedPredecessorDeleteVerdict`, `Get-SafetyNetDeleteVerdict` — hieß bis 0.20.1 `Get-VersionCapDeleteVerdict` und entscheidet für die Versionsgrenze **und** für „Markierte löschen", `Get-SupersededDeletePlan`, `Get-AssignmentHandoverRecord`) und der Leistungsnachweis (`Add-SessionActivity`, `Save-SessionActivity`, `Import-PreviousSessionActivity`, `Get-SessionLeistungstext`) |
