#requires -Version 7
# Darf diese App fallen, wenn nur noch das Sicherheitsnetz dagegen steht?
#
...
#
# Hiess bis 0.20.1 Get-VersionCapDeleteVerdict. Seit 0.21.0 faellt dasselbe Urteil auch fuer die von
# Hand angehakten abgeloesten Apps ("Markierte loeschen"), mit einem anderen Grund fuer dasselbe
# Uebersteuern - die Rechnung kennt den Grund nicht, nur $OverrideInstallations. Die Faelle hier
# gelten deshalb fuer BEIDE Aufrufer.
#
...
#   1. Unbekannt heisst behalten - auch mit Uebersteuern.
#   2. Zuweisungen schuetzen - auch mit Uebersteuern.
...
Describe 'Ohne die Einstellung bleibt alles wie bisher' {
  ...
  It 'loescht, wenn nichts dagegen spricht' {
    $v = Get-SafetyNetDeleteVerdict -AssignmentProbe $script:frei -InstallationProbe $script:leer
  ...
  It 'behaelt eine Fassung, die noch installiert ist' {
    $v = Get-SafetyNetDeleteVerdict -AssignmentProbe $script:frei -InstallationProbe $script:installiert
    $v.Delete | Should -BeFalse
    $v.KeepReason | Should -Be 'installations'
  }

  It 'behaelt eine Fassung, die noch zugewiesen ist' {
    $v = Get-SafetyNetDeleteVerdict -AssignmentProbe $script:zugewiesen -InstallationProbe $script:leer
    $v.Delete | Should -BeFalse
    $v.KeepReason | Should -Be 'assigned'
  }
}

Describe 'Mit Uebersteuern wiegt der Grund des Aufrufers schwerer als Installationen' {
...
Describe 'Die drei Riegel, die auch mit Uebersteuern halten' {
  ...
  It 'Zuweisungen schuetzen auch dann, wenn zusaetzlich Installationen gemeldet sind' {
    ...
    $v = Get-SafetyNetDeleteVerdict -AssignmentProbe $script:zugewiesen -InstallationProbe $script:installiert `
      -OverrideInstallations $true
    $v.Delete | Should -BeFalse
    $v.KeepReason | Should -Be 'assigned'
  }
  ...
  It 'eine unlesbare Installationssondierung schuetzt weiterhin' {
    # Uebersteuert wird eine MELDUNG, nicht ihr Fehlen. Auf Unwissen hin zu loeschen ist das
    # eine, was diese Anwendung nirgends tut.
    $v = Get-SafetyNetDeleteVerdict -AssignmentProbe $script:frei -InstallationProbe $script:blindI `
      -OverrideInstallations $true
    $v.Delete | Should -BeFalse
  ...
  It 'eine unlesbare Zuweisungssondierung schuetzt weiterhin' {
    $v = Get-SafetyNetDeleteVerdict -AssignmentProbe $script:blindA -InstallationProbe $script:leer `
      -OverrideInstallations $true
    $v.Delete | Should -BeFalse
    $v.KeepReason | Should -Be 'unknown'
  }
  ...
  It 'Unbekannt wiegt schwerer als Zugewiesen, damit der Zaehler stimmt' {
    ...
    $v = Get-SafetyNetDeleteVerdict -AssignmentProbe $script:blindA -InstallationProbe $script:installiert `
      -OverrideInstallations $true
    $v.KeepReason | Should -Be 'unknown'
  }
}
...
Describe 'Fehlende Sondierungen' {
  ...
  It 'behandelt $null wie eine Sondierung ohne Antwort' {
    $v = Get-SafetyNetDeleteVerdict -AssignmentProbe $null -InstallationProbe $null -OverrideInstallations $true
    $v.Delete | Should -BeFalse
    $v.KeepReason | Should -Be 'unknown'
  }
}
